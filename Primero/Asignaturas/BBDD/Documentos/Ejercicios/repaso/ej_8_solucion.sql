...
para agrupar por departamento. */
	...
	GROUP BY dept_no;
/*2. Visualizar los departamentos con más de 5 empleados. Utilizar GROUP BY para
agrupar por departamento y HAVING para establecer la condición sobre los grupos.*/
SELECT dept_no,COUNT(*) FROM emple
GROUP BY dept_no
HAVING count(*)>5;
...
/*5. Visualizar el número de vendedores del departamento 'VENTAS' (utilizar la 
...
/*14.  Mostrar los departamentos que tengan más de dos personas trabajando en la 
misma profesión. */
/*15.  A partir de las tablas EMPLE y DEPART mostrar los datos del departamento 
que no tiene ningún empleado. */
/*16.  Mostrar el número de empleados de cada departamento. En la salida se debe
mostrar también los departamentos que no tienen ningún empleado. */
/*17.  Obtener la suma de salarios de cada departamento, mostrando las columnas 
DEPT_NO, SUMA DE SALARIOS y DNOMBRE. En el resultado también se deben mostrar los 
departamentos que no tienen asignados empleados. */
/*18.  Utilizar la función IFNULL (o COALESCE) en la consulta anterior para que 
